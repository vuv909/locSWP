...
export class MyApplicationComponent {
  ...
  constructor(private jobApplication : JobApplicationService, private msg : NzMessageService,private router : Router) {}

  ngOnInit(): void {
    if(StorageService.isLoggedIn() === false){
      this.router.navigateByUrl('/notfound');
    }
    this.jobApplication.viewJobApplicationByEmail(StorageService.getEmail() , 1).subscribe((res)=>{
  ...
  pageActive(pageActive: number): void {
    this.jobApplication.viewJobApplicationByEmail(StorageService.getEmail() , pageActive).subscribe((res)=>{
      this.jobList = res.listResults
      this.totalPage = res.totalPage * 10;
      this.activePage = res.page
    },(error)=>{
